...
class Ch6Ex2Test extends FunSuite {
  test("0 inches should be 0 centimeters") {
    val inches = 0
    assert(InchesToCentimeters(inches) == 0)
  }

  test("0 gallons should be 0 liters") {
    val gallons = 0
    assert(GallonsToLiters(gallons) == 0)
  }

  test("0 miles should be 0 kilometers") {
    val miles = 0
    assert(MilesToKilometers(miles) == 0)
  }

  test("1 inches should be 2.54 centimeters") {
    val inches = 1
    assert(InchesToCentimeters(inches) == 2.54)
  }

  test("1 gallons should be 3.78541 liters") {
    val gallons = 1
    assert(GallonsToLiters(gallons) == 3.78541)
  }

  test("1 miles should be 1.60934 kilometers") {
    val miles = 1
    assert(MilesToKilometers(miles) == 1.60934)
  }

  test("1.5 inches should be 3.81 centimeters") {
    val inches = 1.5
    assert(InchesToCentimeters(inches) == 3.81)
  }
